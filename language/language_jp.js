var data_jp = {
  header_1: "最先端のSTOプラットフォーム",
  header_2: "金融ライセンス５種保有",
  header_3: "info@cryptobamboo.net",
  home: "ホーム ",
  "Crypto Bamboo": "サイト概要",
  solution: "ソリューション",
  project: "プロジェクト",
  "contact us": "お問い合わせ ",
  Visible: "Visible",

  "More Safety": "More Safety",

  Flexible: "Flexible",
  "Control Your Assets": "Control Your Assets",

  info_one: "日本発",
  info_two: "STO特化型のマルチプラットフォーム",

  Regulated: "Regulated",
  Professional: "Professional",
  Reliable: "Reliable",

  advantage: "STOのメリット",
  title_1: "24時間356日",
  detail_1: "伝統的な証券取引所とは違い、STO取引所は休むことなく稼働が可能",
  title_2: "低取引コスト",
  detail_2:
    "配当・譲渡・移転がスマートコントラクトで自動化されるため取引コストが低い",
  title_3: "グローバル市場",
  detail_3:
    "ネットワーク上で取引されるトークンで世界中から資金を呼び込むことが可能",
  title_4: "適法性",
  detail_4: "各国の証券法に準拠した形で発行され資産管理と運用がより安全に",

  AboutUs: "サイト概要",
  about: "Crypto Bambooについて",
  about_detail_1:
    "Crypto Bamboo（クリプトバンブー）は日本発のSTO特化型プラットフォームであり、ブロックチェーンでの新たな金融システム構築を目指しております。",

  about_detail_2:
    "現在、広く知られている仮想通貨は「貨幣」のトークン化として位置づけられていますが、「資産」や「証券」の分野にもトークン化の波が押し寄せています。我々はSTOの発展により、資産の流動性を更に高めることができると信じております。",

  about_detail_3:
    "我々はSTOの発展により、資産の流動性を更に高めることができると信じております。法制、技術発展を促進するため様々なサービスを提供しております。",

  "Get in Touch": "お問い合わせ",
  in_touch_detail:
    "当サービスに関してご不明な点がございましたら、お気軽にお問い合わせ下さい（資料請求をご希望の場合はお問い合わせ内容にご入力下さい）",
  fist_name: "お名前",
  Phone: "電話番号",
  you_message: "お問い合わせ内容",
  "e-mail": "メールアドレス",
  send_message: "送信",
  "The text field is required": "入力されていません",

  Experience: "STOとは",
  Experience_1:
    "STO（Security Token Offering）は株式、債券、不動産などの「資産」をブロックチェーンの技術を通してトークン化することです。",
  Experience_2:
    "一般的に仮想通貨のに「貨幣」の性質を持ったものとは区別されて扱われています。何らかのサービスに対する利用料として使用されるものがユーティリティトークン（Utility Token）と呼ばれるのに対して、証券としての性質を有したものはセキュリティトークン（Security Token）と呼ばれています。",
  Experience_3:
    "また、詐欺事件などで問題が指摘されたICO（Initial coin offering）などと比べると、各国の証券法を遵守した形で発行されるSTOは比較的安全な資金調達方法として注目されています。",

  Links: "リンク",
  "Why Choose Us": "トークンによる資金調達",
  choose_us_detail:
    "トークンでの資金調達はブロックチェーン技術の発達により世界中で急激に成長しています。また、Ethereum(イーサリアム)のトークン規格の開発により、全世界で統一された資金調達方式が広がっています。",
  choose_us_detail_2:
    "これにより今まで国境を超えることがなかった資産がボーダレスに移動するようになりました。2018年には世界で約215億ドルの資金がトークンで集められています。",
  "View statistics": "統計を見る",

  num_1: "1",
  num_detai_1: "日本初のSTO",
  num_detai_1_1: "プラットフォーム",
  num_2: "5",
  num_detai_2: "会社設立から５年",
  num_3: "4",
  num_detai_3: "193ヶ国のSTO案件に対応可能",
  num_4: "5",
  num_detai_4: "金融ライセンスを５種類保有",

  service: "ソリューション",
  "Our production": "総合的バックアップ",

  "DIGITAL ADVERTISING": "信頼の金融ライセンス",
  "MEDIA BUYING": "流動性の回復",
  "BRAND MARKETING": "資産の統合運用",
  "MARKET RESEARCH": "プロフェッショナル",
  service_title: "金融商品取引業・仮想通貨取引に関わるライセンスを全て保有",
  service_item1: "・仮想通貨交換業者",
  service_item2: "・第一種金融商品取引業者",
  service_item3: "・第二種金融商品取引業者",
  service_item4: "・投資運用業者",
  service_item5: "・投資助言・代理業者",

  service_title2: "流動性の回復",
  service_item11:
    "・伝統的な資金調達方法は流動性において様々な課題がありました。ベンチャーキャピタルなどで投資を考える場合、自分の資産が長期間動かせないことを考慮する必要があります。",
  service_item12:
    "・これらの流動性を解決するのが、STOです。ブロックチェーンに記帳された資産記録はクロスボーダーでの取引を容易にし、保有期間、配当、移転の条件などは予めスマートコントラクトで自由に記述が可能です。",
  service_item13:
    "・我々はこれらの技術を活用して、世界の経済に再び流動性を生み出します。",

  service_title3: "資産の統合運用",
  service_item21:
    "ブロックチェーンは様々な資産、価値を最小単位でトークン化できます。どんなに小さな経済的価値も全てトークンエコノミーに統合され、様々なトークンが互いに組み合わさることで、今までに無かった資産運用が可能となります。",
  service_item22:
    "イーサリアムで開発されたERC-1400と呼ばれる規格は、発行主体がセキュリティトークンの提供を有効的に管理できるよう設計されました。これにより、各国の関連法に合致するという保証が得られ、運用も容易となります。",

  service_title4: "プロフェッショナル",
  service_item31:
    "クリプトバンブーはブロックチェーン、金融のプロフェッショナルで構成されています。従来の金融とブロックチェーンが融合した世界は無限大の可能性を秘めています。",
  service_item32:
    "トークン発行をより簡易に行うために熟練エンジニアチームがSTOをサポートします。法律面では経験豊富なコンサルタントが安全で着実な運用方法をアドバイスします。",

  service_item1_detail: "  トークン発行を技術的にサポート",
  service_item2_detail: "証券トークン化・レバレッジ取引",
  service_item3_detail: "STO資金調達のインフラ構築",
  service_item4_detail: "ファンド資金進出を促進",
  service_item5_detail: "STOに関する包括的なコンサルティング",

  History: "トークン資金調達の動向",
  history_title_1: "2014年 イーサリアムの大型資金調達",
  history_ditail_1: "プロジェクトのためICOを実施し、2週間で1500万ドルを調達",

  history_title_2: "2019年 日本で本格的な法制化",
  history_ditail_2:
    "金融庁による資金決済法と金融商品取引法の改正案が通常国会に提出",

  history_title_3: "2017年 ICO元年",
  history_ditail_3: "トークンによる資金調達が爆発的に普及",
  history_ditail_3_1: "数百件のＩＣＯが実施され、約４５００億円が集まる",

  Statistics: "データ",

  "Our Targets": "資産のトークン化・資金調達を我々がバックアップします",
  Targets_detail:
    "アセットマネジメントをより効率的、安全にすることが我々の使命です。豊富な経験と知識で、お客様の資金調達・運用を全力でサポート致します。お気軽にお問い合わせ下さい。",
  "Our Certificates": "STOに関する情報共有",
  Certificates_detail:
    "セキュリティトークンは急速な勢いで技術発展が起こっています。クリプトバンブーではSTOの法規制、発展動向に関する最新情報を発信致します。",

  button_1: "お問い合わせ",
  button_2: "ソリューション",

  statistics_second_title: "2018年に行われたSTO件数",
  statistics_second_item_title: "ICOからSTOへ",
  statistics_second_item_1:
    "2017年はICO元年、2018年はSTO元年と言われています。",
  statistics_second_item_2:
    "世界で多くのSTOプロジェクトが生まれ、法令で定められた審査を通ったものが資金を集めやすくなっています。",
  statistics_second_item_3:
    "無秩序に行われるICOが各国で禁止されるのに対して、STOは政府の支持を受けやすい傾向にあります。"
};
